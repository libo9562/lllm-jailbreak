...
class JailbreakResult:
    ...
    def __init__(
        self, technique: str, success: bool, responses: List[Dict[str, Any]], metadata: Optional[Dict[str, Any]] = None
    ) -> None:
...
class DeepinceptionJailbreaker(BaseJailbreaker):
    ...
    def __init__(self, target_model: str = "llama3.2:1b", **kwargs) -> None:
    ...
    def execute(
        self, goal: Optional[str] = None, exp_name: str = "test", defense: str = "none", **kwargs
    ) -> JailbreakResult:
        """
        ...
        try:
            ...
            data_path = os.path.join(os.path.dirname(__file__), "Deepinception", "res", f"data_{exp_name}.json")
            ...
            # Initialize model
            target_lm = self.load_models("Ollama", self.target_model, {"target_max_n_tokens": self.max_tokens})
            ...
            for idx, item in enumerate(data):
                ...
                item_result = {"topic": item["topic"], "qA_pairs": []}
            ...
            success = len(results) > 0 and any(len(item["qA_pairs"]) > 0 for item in results)
            ...
            return JailbreakResult(
                technique=self.get_technique_name(), success=False, responses=[], metadata={"error": str(e)}
            )
...
class PAIRJailbreaker(BaseJailbreaker):
    ...
    def __init__(self, target_model: str = "gpt-3.5-turbo", attack_model: str = "gpt-3.5-turbo", **kwargs) -> None:
        super().__init__(target_model, **kwargs)
    ...
    def execute(self, goal: str, target_str: Optional[str] = None, **kwargs) -> JailbreakResult:
        """
...
class UnifiedJailbreaker:
    ...
    def execute(
        self, technique: Union[str, JailbreakTechnique], goal: Optional[str] = None, **kwargs
    ) -> JailbreakResult:
...
def main() -> None:
    ...
    # Technique selection
    parser.add_argument(
        "--technique", choices=["deepinception", "pair"], required=True, help="Jailbreaking technique to use"
    )

    # Common parameters
    parser.add_argument("--target-model", default="llama3.2:1b", help="Target model name")
    parser.add_argument("--goal", type=str, help="Jailbreaking goal (required for PAIR, optional for Deepinception)")
    ...
    parser.add_argument("--defense", default="none", choices=["none", "sr", "ic"], help="Defense mechanism")
    parser.add_argument("--target-max-n-tokens", type=int, default=300, help="Maximum tokens for target model")

    # PAIR specific
    parser.add_argument("--attack-model", default="vicuna-13b-v1.5", help="Attack model for PAIR")
    parser.add_argument("--target-str", type=str, help="Target response string for PAIR")
    parser.add_argument("--n-streams", type=int, default=3, help="Number of concurrent streams for PAIR")
    parser.add_argument("--n-iterations", type=int, default=3, help="Number of iterations for PAIR")
    ...
    kwargs = {"target_model": args.target_model, "target_max_n_tokens": args.target_max_n_tokens}

    if args.technique == "deepinception":
        kwargs.update({"exp_name": args.exp_name, "defense": args.defense})
