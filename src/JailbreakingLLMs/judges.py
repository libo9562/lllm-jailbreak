...
class JudgeBase:
    ...
    def get_judge_prompt(self, attack_prompt, target_response) -> str:
        return f"[PROMPT]:{attack_prompt}\n[ASSISTANT'S RESPONSE]: {target_response}"

    def score(self, prompt_list, response_list) -> Never:
        raise NotImplementedError
...
class GPTJudge(JudgeBase):
    ...
    def score(self, attack_prompt_list, target_response_list):
        convs_list = [
            ...
            for prompt, response in zip(attack_prompt_list, target_response_list, strict=False)
        ]
        raw_outputs = self.judge_model.batched_generate(
            convs_list, max_n_tokens=self.max_n_tokens, temperature=self.temperature, top_p=1
        )
...
class OpenSourceJudge(JudgeBase):
    def __init__(self, judge_model, judge_tokenizer, args) -> None:
