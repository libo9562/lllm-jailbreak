...
def set_logger_level(logger, verbosity) -> None:
    if verbosity == 0:
...
class WandBLogger:
    ...
    def __init__(self, args, system_prompts) -> None:
    ...
    def log(self, iteration: int, attack_list: list, response_list: list, judge_scores: list) -> None:
        ...
        if any([score == 10 for score in judge_scores]):
            ...
            if not self.is_jailbroken:
                self.query_to_jailbreak = (
                    self.batch_size * (iteration - 1) + jailbreak_ind + 1
    ...
    def print_summary_stats(self, iter) -> None:
        bs = self.batch_size
        df = self.table
    ...
    def print_final_summary_stats(self) -> None:
