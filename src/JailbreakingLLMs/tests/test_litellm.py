...
class TestVicuna(unittest.TestCase):
    ...
    def test_get_response(self) -> None:
        ...
        correct_responses = ["The ocean, a vast and endless sea\n", "What is 1+1?"]

        num_convs = len(convs_list)
        responses = self.lm.batched_generate(convs_list, max_n_tokens=10, temperature=0, top_p=0.9)
        self.assertEqual(len(responses), num_convs)
        self.assertEqual(responses, correct_responses)

    def test_initialized_response(self) -> None:
        convs_list = [
            [
            ...
            [
                ...
                {
                    "role": "assistant",
                    "content": "I'm bad at writing poems, but here goes: It was a dark and stormy night.",
                },
            ],
        ]
        correct_responses = ["3,4,5,6,7,", "The waves were crashing, the wind was how"]
        num_convs = len(convs_list)
        responses = self.lm.batched_generate(convs_list, max_n_tokens=10, temperature=0, top_p=0.9)
        self.assertEqual(len(responses), num_convs)
        self.assertEqual(responses, correct_responses)
