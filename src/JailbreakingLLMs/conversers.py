...
class AttackLM:
    ...
    def __init__(
        self, model_name: str, max_n_tokens: int, max_n_attack_attempts: int, category: str, evaluate_locally: bool
    ) -> None:
    ...
    def preprocess_conversation(self, convs_list: list, prompts_list: list[str]):
        ...
        for conv, prompt in zip(convs_list, prompts_list, strict=False):
    ...
    def _generate_attack(self, openai_conv_list: list[list[dict]], init_message: str):
        ...
        for attempt in range(self.max_n_attack_attempts):
            ...
            for i, full_output in enumerate(outputs_list):
                ...
                full_output = (
                    init_message + full_output + "}"
                )  # Add end brace since we terminate generation on end braces
    ...
    def get_attack(self, convs_list, prompts_list):
        ...
        for jailbreak_prompt, conv in zip(new_adv_prompts, convs_list, strict=False):
...
class TargetLM:
    ...
    def get_response(self, prompts_list):
        if self.use_jailbreakbench:
            llm_response = self.model.query(
                prompts=prompts_list, behavior=self.category, phase=self.phase, max_new_tokens=self.max_n_tokens
            )
            ...
            responses = self.model.batched_generate(
                full_prompts, max_n_tokens=self.max_n_tokens, temperature=self.temperature, top_p=self.top_p
            )
